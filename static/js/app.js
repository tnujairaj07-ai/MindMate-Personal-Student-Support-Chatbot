...
  const subject = window.prompt("Which subject do you want a study plan for? (e.g. DBMS)");
  if (!subject) return;


  // Show user message in chat
  appendMessage("Please create a study plan for " + subject, "user");
...
      body: JSON.stringify({
        subject,
        hours_per_day: 2, // later you can make this configurable
      }),
    });
    const data = await res.json();
...
    }
    appendMessage(data.plan, "bot");
  } catch (e) {
    console.error("Study plan error:", e);
    appendMessage(
      "Sorry, I couldn't generate a study plan right now. Please try again later.",
      "bot"
    );
  }
}
...
    if (nameInput) nameInput.value = p.name || "Student User";
    if (emailInput) emailInput.value = p.email || "";
    if (courseInput) courseInput.value = p.course || "";
    if (semInput) semInput.value = p.semester != null ? p.semester : "";
    if (rollInput) rollInput.value = p.roll_no || "";
  } catch (e) {
    console.error("Failed to load profile:", e);
    if (statusEl) {
      statusEl.style.color = "#f97316";
      statusEl.textContent = "Could not load profile. Please try again.";
    }
  }
}
...
  if (!nameInput || !courseInput || !semInput || !rollInput || !statusEl) return;
...
      headerInfo.textContent = currentUser.name + " (" + currentUser.role + ")";
    }
  } catch (e) {
